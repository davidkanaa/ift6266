...
class DCGAN(object):
    ...
    def _build(self):
        ...
        # discriminate real images, fake images
    ...
    def _decoder(self, code, embeddings, is_training, interpolation=False):
        with tf.variable_scope("decoder") as scope:
            ...
            if interpolation:
                ...
                h0 = deconv2d(inputs=z,
                              ...
                              name="deconv-0")
                # 4x4x256 --> 8x8xx256
                h0 = tf.image.resize_bicubic(h0, [8, 8], name="interpol-0")
                ...
                h1 = deconv2d(inputs=h0,
                              ...
                              name="deconv-1")
                # 8x8x128 --> 16x16x128
                h1 = tf.image.resize_bicubic(h1, [16, 16], name="interpol-1")
                ...
                h2 = deconv2d(inputs=h1,
                              ...
                              name="deconv-2")
                # 16x16x64 --> 32x32x64
                h2 = tf.image.resize_bicubic(h2, [32, 32], name="interpol-2")
    ...
    def train(self, datasets, batch_size=20, max_epoch=50, optimizer=tf.train.AdamOptimizer, clip_coef=1):
        ...
        l2_opt = self._optimizer(self._losses.get("l2"), clip_coef, optimizer)
        gen_opt = self._optimizer(self._losses.get("generator"), clip_coef, optimizer)
        disc_opt = self._optimizer(self._losses.get("discriminator"), clip_coef, optimizer)
        ...
        for epoch in np.arange(1, max_epoch + 1):
            ...
            for batch_index in np.arange(n_train_batches):
                ...
                op, d_loss, _, __, ___ = self._session.run(fetches=[self._op,
                ...
                for k in range(2):
                    op, g_loss, _, __, ___ = self._session.run(fetches=[self._op,
    ...
    def apply(self, images, embeddings=None):
        # build the model if not built
        if not self._built:
            self._build()
        imgs = images.reshape([-1] + self._images_shape)
        if embeddings is None:
            o = self._session.run(fetches=self._outputs, feed_dict={self._inputs: imgs,
                                                                    self._is_training: False})
        else:
            o = self._session.run(fetches=self._outputs, feed_dict={self._inputs: imgs,
